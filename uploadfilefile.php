...
if($_GET['export'] == 'true'){
$query = mysqli_query($conn, 'select * from uploadfile'); // Get data from Database from demo table
 
 
    $delimiter = ",";
    $filename = "uploadfile" . date('dmY') . ".csv"; // Create file name
...
'Final_Success_Rate','CELL_DB_EXCEPTION');
    ...
    while($row = $query->fetch_assoc()){
        ...
        $lineData = array($row['DATE'], $row['ATTEMPT'], $row['FAILURE'], $row['SUCCESS'], $row['COUNT_OF_CONSENT_PENDING'],
						  ...
						  $row['Phone_Switched_OFF'], $row['Out_of_coverage'], $row['Total_Success'], $row['Final_Success_Rate'], $row['CELL_DB_EXCEPTION']);
